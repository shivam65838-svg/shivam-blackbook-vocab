...
export function useColorScheme() {
  ...
  const [appearance, setAppearance] = useState(() => readJson(STORAGE_KEYS.settings, { appearance: "system" }).appearance || "system");

  useEffect(() => {
    setHasHydrated(true);
    const sync = () => setAppearance(readJson(STORAGE_KEYS.settings, { appearance: "system" }).appearance || "system");
